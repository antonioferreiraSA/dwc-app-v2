...
import { channelID, ytkey } from '../../config'// Import your environment variables
...
const VideosScreen = ({ navigation }) => {
  ...
  const loadVideos = () => {
    ...
    fetch(`https://www.googleapis.com/youtube/v3/search?key=${ytkey}&channelId=${channelID}&part=snippet,id&order=date&maxResults=50`)
      ...
      .catch(error => {
        ...
        // Handle errors and ensure refreshing state is set to false
        setRefreshing(false);
      });
  };
  ...
  return (
    <View style={[styles.mainContainer, { backgroundColor: colorScheme.container }]}>
      <Text XXL bold style={[styles.headerTitle, { color: colorScheme.text }]}>Previous Sermons</Text>
      <ScrollView
        ...
        {loading ? (
          ...
          videos.map(video => (
            <TouchableOpacity key={video.id} style={styles.videoBlock} onPress={() => handleVideoPress(video.id, video.title)}>
              ...
              <FontAwesome name="play-circle-o" size={100}  color="rgba(255, 255, 255, 0.5)" style={styles.playIcon} />
            </TouchableOpacity>
          ))
        )}
      </ScrollView>
